# reuse
Java 전체를 관통하는 키워드 중에 하나는 **코드의 재사용성**이라고 생각합니다.  
인스턴스 필드, 다형성, 제네릭, 인터페이스, 의존성 주입은 모두 코드의 재사용성을 높이는 것이 목적이기 때문입니다.  
그래서 우리가 Java를 사용하는 중요한 이유 중에 하나는 코드의 재사용성을 극대화하는 것입니다.  
