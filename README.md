...
그러나 상속은 부모와 자식간의 결합도가 높다. 부모가 수정되면 자식도 영향을 받기 때문이다. 그래서 '코드 재사용성을 높이기 위해 코드 간에 관계를 맺는 것은 곧 **의존 관계를 맺는 것**'이라 보면 될 것이다.  
...
이제 모든 단점을 해결했다. 결론은, 객체를 의존하는 가장 좋은 방법은 **DI와 다형성을 결합하는** 것이다.
